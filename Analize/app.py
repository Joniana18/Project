...
from scraper_pyppeteer import scrape_all_jobs
...
use_pyppeteer = st.checkbox("Use pyppeteer for dynamic content (slower but more reliable for JavaScript-heavy sites)")

if st.button("Scrape Jobs"):
    if url:
        st.write("Scraping data... Please wait.")
        if use_pyppeteer:
            job_data = scrape_all_jobs(url, site_type)
        else:
